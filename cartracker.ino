...
void loop() {
    ...
    // haversine function to calculate distance travelled
    if (lon1 == 0.0){
        lon1 = t.readLon();
    }
    if (lat1 == 0.0){
    ...
    dlon = lon2 - lon1;
    dlat = lat2 - lat1;
    a = pow(sin(dlat * pi/180/2),2) + cos(lat1*pi/180) * cos(lat2*pi/180) * pow(sin(dlon*pi/180/2),2);
    c = 2 * asin(sqrt(a));
    d = c * r;
    if (d > mindist){
        lon1 = lon2;
